...
class FlowmatchingActionHeadConfig(PretrainedConfig):
    ...
    inference_seed: int = field(default=0, metadata={"help": "Deterministic seed for diffusion inference noise."})
...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        self.inference_seed = getattr(config, "inference_seed", None)
    ...
    def predict_action(self, vl_embs: torch.Tensor, state: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.inference_seed is not None:
            generator = torch.Generator(device=vl_embs.device)
            generator.manual_seed(int(self.inference_seed))
