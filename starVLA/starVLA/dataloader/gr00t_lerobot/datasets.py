    ...
    *,
    ...
    while True:
        ...
        if time.monotonic() - start_time > timeout_seconds:
            raise TimeoutError(f"Timed out waiting for {description} to be ready")
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        if le_statistics is None:
            _wait_for_cache(
                lambda: _load_stats_cache(
                    stats_path,
                    stats_cache_config,
                    invalidate_legacy=False,
                )
                is not None,
                description=f"dataset statistics cache at {stats_path}",
            )
            le_statistics = _load_stats_cache(
                stats_path,
                stats_cache_config,
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        if not is_main():
            _wait_for_cache(
                lambda: steps_path.exists(),
                description=f"steps cache at {steps_path}",
            )
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        if self.data_cfg is not None:
            default_resolution = self.data_cfg.get("default_image_resolution", None)
            if default_resolution is not None:
                if len(default_resolution) == 3:
                    target_size = (int(default_resolution[-1]), int(default_resolution[-2]))
                elif len(default_resolution) == 2:
                    target_size = (int(default_resolution[-1]), int(default_resolution[-2]))
