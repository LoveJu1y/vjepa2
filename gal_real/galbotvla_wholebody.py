...
class GalbotVLA:
    def __init__(self, args: Args):
        ...
        for i in range(len(args.host)):
            self.model_agent.append(ModelAgent(args.host[i], args.port[i], args.action_horizon))
            self.flag_infer_thread_is_run.append(0)
            self.flag_model_first_infer.append(True)
            self.last_action_time_delay.append(0)
            self.model_agent[i].ws_client.network_latency_tolerance = self.args.network_latency_tolerance
    ...
    def run(self, args: Args):
        ...
        time.sleep(0.1) #有时候存在在同一个singorix周期里，同时有service和topic消息，有时候会先触发service后topic，导致错误，故这里sleep(0.1)，后期底层修改后可删
        ...
        while True:
            ...
            if res == "stop":
                ...
                time.sleep(0.1)
            if time.perf_counter() - t0 > self.args.action_horizon_use * self.args.dt_model_control + 1:
    ...
    def blocking_mode_training_time_RTC(self):
        ...
        response = None
        obs, obs_visualization = self.galbot.get_obs_wholebody_compressed(self.flag_model_first_infer[0], 0, response)
        ...
        rate = ToolRate(1.0/(self.args.dt_model_control*self.args.action_horizon_use))
    ...
    def blocking_mode(self):
        while not self.shutdown_event.is_set():
            ...
            obs, obs_visualization = self.galbot.get_obs_wholebody_compressed(self.flag_model_first_infer[0], 0)
            if self.flag_model_first_infer[0]:
                self.flag_model_first_infer[0] = False
    ...
    def nonblocking_mode(self):
        while not self.shutdown_event.is_set():
            if (not self.flag_has_moved_to_init_pose) and sum(self.flag_infer_thread_is_run) == 0:  #如果没初始化，且没有模型在跑，走初始化
                self.galbot.move_to_init_pose_wholebody()
                self.vla_cost_time = 0
                self.flag_has_moved_to_init_pose = True
            ...
            for i in range(len(self.args.host)):
                if (self.flag_infer_thread_is_run[i] == 0 and self.flag_has_moved_to_init_pose and self.galbot.error_imformation == ""):  #初始化后，启动模型
                    self.flag_infer_thread_is_run[i] = 1
                    thread_ = threading.Thread(target=self.infer_thread, args=(i,))
                    thread_.start()

            if (self.check_stop() and self.args.auto_stop and self.flag_has_moved_to_init_pose and sum(self.flag_infer_thread_is_run) == len(self.args.host) and self.galbot.error_imformation == "" ):  #回退到位，检测是否成功
                with self.lock_exit:
                    for i in range(len(self.args.host)):  # 停止推理线程
                        self.flag_infer_thread_is_run[i] = -1  # -1表示退出  0表示未运行  1表示正在运行

                q_wholebody = self.galbot.galbot_interface.pose_buffer[1]
                if q_wholebody[22] > self.args.success_gripper_width:  # 检测到有物体，成功并退出
                    self.logger.info( "success, gripper width is " + str(q_wholebody[22]) + "m > " + str(self.args.success_gripper_width) + "m" )
                    self.shutdown_event.set()
                    self.galbot.shutdown()
                    break
                elif self.args.allow_retry: #未检测到物体，允许重试时，进行重试
                    self.logger.error( "start retry, right gripper width is " + str(q_wholebody[22]) + "m <=" + str(self.args.success_gripper_width) + "m" )
                    self.flag_has_moved_to_init_pose = False
                    self.galbot.args.has_init_action = True
                    self.grasp_count = self.grasp_count + 1
                    if self.grasp_count >= self.args.retry_fail_max_num:  #超过最大重试次数，失败并退出
                        self.logger.error(f"retry exceeded the limit {self.args.retry_fail_max_num} times, ")
                        self.galbot.error_imformation = ( self.galbot.error_imformation + f"retry exceeded the limit {self.args.retry_fail_max_num} times, " )
                else:  #未检测到物体，不允许重试，失败并退出
                    self.galbot.error_imformation = ( self.galbot.error_imformation + "failed, right gripper width is " + str(q_wholebody[22]) + "m <=" + str(self.args.success_gripper_width) + "m" )
                    self.logger.error( "failed, right gripper width is " + str(q_wholebody[22]) + "m <=" + str(self.args.success_gripper_width) + "m" )

            if self.args.auto_stop and self.vla_cost_time > self.args.vla_max_cost_time:  #执行超时
                self.vla_cost_time = 0
                with self.lock_exit:
                    for i in range(len(self.args.host)):  # 停止推理线程
                        self.flag_infer_thread_is_run[i] = -1  # -1表示退出  0表示未运行  1表示正在运行
                ...
                if self.args.allow_retry:  #超时，允许重试时，进行重试
                    self.logger.error("start retry, " + f"vla_cost_time more than {self.args.vla_max_cost_time}s")
                    self.flag_has_moved_to_init_pose = False
                    self.galbot.args.has_init_action = True
                    self.grasp_count = self.grasp_count + 1
                    if self.grasp_count >= self.args.retry_fail_max_num:  #超过最大重试次数，失败并退出
                    ...
                    self.galbot.error_imformation = ( self.galbot.error_imformation + f"vla_cost_time more than {self.args.vla_max_cost_time}s" )
    ...
    def infer_thread(self, i):
        while self.flag_infer_thread_is_run[i] == 1:
            ...
            obs, obs_visualization = self.galbot.get_obs_wholebody_compressed( self.flag_model_first_infer[i], round(self.last_action_time_delay[i] * 1000 * (0.1 / self.args.dt_model_control)) )
            ...
            try:
                ...
                error_msg = f"vla model error (thread {i}): {type(e).__name__}: {str(e)}"
                self.logger.error(error_msg)
                self.galbot.error_imformation = self.galbot.error_imformation + error_msg + ", "
                self.vla_model_error = True
                response = {}
            ...
            with self.lock_exit:
                action_time_delay = time.time() - obs_visualization[OBS_STATE + "_timestamp"] 
                if self.flag_infer_thread_is_run[i] == 1 and (not self.vla_model_error):
                    if self.flag_model_first_infer[i]:
                        self.galbot.pub_wholebody_command(obs, obs_visualization, response, 0)
                    else:
                        self.galbot.pub_wholebody_command(obs, obs_visualization, response, action_time_delay)
    ...
    def check_stop(self):
        ...
        if len(self.args.object_name) == 1:
            if ( np.linalg.norm(right_end_T[0:3, 3:4] - right_end_T_deepest[0:3, 3:4]) > dis ) or (np.linalg.norm(left_end_T[0:3, 3:4] - left_end_T_deepest[0:3, 3:4]) > dis):
                return True
            else:
                return False
        elif len(self.args.object_name) == 2:
            if ( np.linalg.norm(right_end_T[0:3, 3:4] - right_end_T_deepest[0:3, 3:4]) > dis ) and (np.linalg.norm(left_end_T[0:3, 3:4] - left_end_T_deepest[0:3, 3:4]) > dis):
                return True
            else:
                return False
        else:
    ...
    def change_control_mode(self):
        ...
        if self.args.vla_type == "VLA_training_time_RTC":
            ...
            self.galbot.error_imformation = self.galbot.error_imformation + "unsupported vla_type: " + self.args.vla_type + ", "
            return False
        ...
        while True:
            ...
            if self.args.vla_type == "VLA_training_time_RTC" and res == "PlanJoint_Vla_TrainingRTC":
                change_success = True
                break
            elif self.args.vla_type == "VLA" and res == "PlanJoint_Vla_WeightingPosByAccCtrl":
                change_success = True
                break
            elif self.args.vla_type == "PlanEndEffector_Vla_WeightingPosByAccCtrl" and res == "PlanEndEffector_Vla_WeightingPosByAccCtrl":
                change_success = True
                break
            else:
                time.sleep(0.1)
            if time.perf_counter() - t0 > 1:
                ...
                self.galbot.error_imformation = self.galbot.error_imformation + "change_control_mode time out, "
                break
    ...
    def shutdown(self):
        with self.lock_exit:
            if (not self.args.blocking) and sum(self.flag_infer_thread_is_run) == len(self.args.host):
                for i in range(len(self.args.host)):
                    self.flag_infer_thread_is_run[i] = -1
        self.shutdown_event.set()
        ...
        while True:
            ...
            if res == "stop":
                ...
                time.sleep(0.1)
            if time.perf_counter() - t0 > self.args.action_horizon_use * self.args.dt_model_control + 1:
                self.logger.info("shutdown exit vla request_vla_service('stop') time out stop ")
                break


if __name__ == "__main__":
    args = Args()
    galbotvla = GalbotVLA(args)
    ...
    for i in range(len(args.host)):
        tool_shutdown.on_shutdown(galbotvla.model_agent[i].ws_client.conn.close)
